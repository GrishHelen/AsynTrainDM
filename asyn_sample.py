...
def main(_):
    ...
    for idx in tqdm(
        ...
        if config.generate_dm or config.static_mask:
            ...
            if config.generate_dm:
                ...
                for j, image in enumerate(images):
                    ...
                    pil.save(os.path.join(save_dir, f"images/{(j + global_idx):05}_DM.png"))
        ...
        if config.generate_dm_concave:
            ...
            for j, image in enumerate(images):
                pil = Image.fromarray((image.cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8))
                pil.save(os.path.join(save_dir, f"images/{(j + global_idx):05}_dm_concave.png"))
        ...
        for j in range(item_cnt):
            state_prev_t_binary.append(
                cross_mask[:, j] * func_prev_binary(state_t, config.sample.num_steps, k=item_k_list[prompt_idx][j]))
        ...
        for i in tqdm(
            ...
            with autocast():
                with torch.no_grad():
                    ...
                    for j in range(item_cnt):
                        state_prev_t_binary.append(
                            cross_mask[:, j] * func_prev_binary(state_t, config.sample.num_steps - i - 1,
