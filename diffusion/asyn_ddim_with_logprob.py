...
def ddim_step_with_logprob(
    ...
    prev_timestep = self.timesteps[timestep_idx + 1] if timestep_idx + 1 < self.timesteps.shape[0] else torch.tensor(0,
                                                                                                                     dtype=torch.int64,
...
def asyn_ddim_step_with_logprob(
        self: DDIMScheduler,
        model_output: torch.FloatTensor,
        timestep,
        prev_timestep,
    ...
    alpha_prod_t = self.alphas_cumprod[timestep.cpu()].to(sample.device)
    alpha_prod_t_prev = self.alphas_cumprod[prev_timestep.cpu()].to(sample.device)
